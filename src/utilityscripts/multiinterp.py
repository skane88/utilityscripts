...
def multi_interp(
    *,
    ...
    xp: list[float] | np.ndarray,
    fp: list[float] | list[list[float]] | np.ndarray,
    ...
    Notes
    -----
    This is NOT 2D interpolation. It is simply repeated 1d interpolation of the
    ...
    x : Union[float, List[float], np.ndarray]
        The x points to interpolate.
    xp : Union[List[float], np.ndarray]
        The original x data points. Must be sorted. Raises a value error if not
        sorted.
    fp : Union[List[float], List[List[float]], np.ndarray]
        The original y data points to interpolate.
    left : float | None
        An optional fill value to use if a value in x is less than the values
        in xp (and thus cannot be interpolated). If None, the value of fp at min(xp)
        will be used.
    right : float | None
        An optional fill value to use if a value in x is greater than the
